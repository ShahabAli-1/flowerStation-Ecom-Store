document.addEventListener("DOMContentLoaded", function () {
  ...
  const totalImagesWidth =
    (imageWidth + gap) * scrollingImages.querySelectorAll("img").length;
  ...
  scrollingImages.addEventListener("scroll", () => {
    const newImageIndex =
      Math.floor((scrollingImages.scrollLeft + gap) / (imageWidth + gap)) %
      numImages;
    if (newImageIndex !== currentScrollIndex) {
      currentScrollIndex = newImageIndex;
      updateLargeImage(
        scrollingImages.querySelectorAll("img")[newImageIndex].src
      );
    }
  });
